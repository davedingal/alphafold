...
if [[ $# -eq 0 ]]; then
    echo "Error: download directory and tar directory must be provided as an input argument."
    exit 1
fi

DOWNLOAD_DIR="$1"
TAR_DIR="$2"
DOWNLOAD_MODE="${3:-full_dbs}" # Default mode to full_dbs.
...
bash "${SCRIPT_DIR}/download_alphafold_params.sh" "${DOWNLOAD_DIR}" "${TAR_DIR}"

if [[ "${DOWNLOAD_MODE}" = reduced_dbs ]] ; then
  echo "Downloading Small BFD..."
  bash "${SCRIPT_DIR}/download_small_bfd.sh" "${DOWNLOAD_DIR}"  "${TAR_DIR}"
else
  echo "Downloading BFD..."
  bash "${SCRIPT_DIR}/download_bfd.sh" "${DOWNLOAD_DIR}"  "${TAR_DIR}"
fi

echo "Downloading MGnify..."
bash "${SCRIPT_DIR}/download_mgnify.sh" "${DOWNLOAD_DIR}" "${TAR_DIR}"

echo "Downloading PDB70..."
bash "${SCRIPT_DIR}/download_pdb70.sh" "${DOWNLOAD_DIR}" "${TAR_DIR}"

echo "Downloading PDB mmCIF files..."
bash "${SCRIPT_DIR}/download_pdb_mmcif.sh" "${DOWNLOAD_DIR}" "${TAR_DIR}"

echo "Downloading Uniclust30..."
bash "${SCRIPT_DIR}/download_uniclust30.sh" "${DOWNLOAD_DIR}" "${TAR_DIR}"
